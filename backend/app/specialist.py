...
def all_specialist():
    ...
    if sort_by == "rating":
        ...
        query = query.outerjoin(
            rating_subq, Specialist.id == rating_subq.c.specialist_id
        )
...
def specialist_profile(specialist_id):
    ...
    specialist = Specialist.query.filter_by(
        id=specialist_id, is_active=True, is_approved=True
    ).first()
    if not specialist:
...
def get_my_documents():
    ...
    docs = (
        SpecialistDocuments.query.filter_by(specialist_id=specialist.id)
        .order_by(SpecialistDocuments.uploaded_at.desc())
        .all()
    )
    result = []
    for doc in docs:
